// Supabase Edge Function: create-driver-virtual-account
// Creates a BudPay dedicated virtual account for the calling driver and stores
// the result on driver_profile (wallet_account, bank_name, account_name).
//
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Admin client to read/write driver_profile regardless of RLS.
    ...
    // Step 1: create (or reuse) a BudPay customer for this driver.
    ...
    if (!customerCode) {
      ...
      await supabaseAdmin.from('driver_profile').update({ budpay_customer_code: customerCode }).eq('uuid', user.id);
    }
    ...
    // Step 3: persist the account details on the driver profile.
    ...
    if (updateError) {
      console.log('Unable to save virtual account to driver profile:', updateError);
    ...
    console.log('create-driver-virtual-account error:', error);

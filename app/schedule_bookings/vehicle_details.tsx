...
const computeFareBreakdown = (pricing: VehiclePricingRow, distanceKm: number, durationMin: number) => {
  ...
  const totalFare = roundCurrency(amount + vatAmount + stateLevy);
...
export default function ScheduledVehicleDetails() {
  const router = useRouter();
  const { pickup, dropoff, pickupLat, pickupLng, dropoffLat, dropoffLng, stops, rideFor, guestRiderName, guestRiderNumber, vehiclePricing, scheduledDate, scheduledTime, passengerCount, scheduleType } = useLocalSearchParams<{
    pickup?: string;
    dropoff?: string;
    pickupLat?: string;
    pickupLng?: string;
  ...
  const resolvedScheduledDate = normalizeRouteParam(scheduledDate);
  const resolvedScheduledTime = normalizeRouteParam(scheduledTime);
  const formattedScheduledTime = formatDisplayTime(resolvedScheduledTime);
  const resolvedPassengerCount = normalizeRouteParam(passengerCount);
  const resolvedScheduleTypeParam = normalizeRouteParam(scheduleType);
  const resolvedScheduleType: 'ride' | 'hourly' | 'hire' =
    resolvedScheduleTypeParam === 'hourly' || resolvedScheduleTypeParam === 'hire'
      ? resolvedScheduleTypeParam
      : 'ride';
  ...
  const navigateToGetDriver = async () => {
    console.log('[ScheduledBooking] Book button pressed', {
    ...
    if (!resolvedScheduledDate.trim() || !resolvedScheduledTime.trim()) {
      Alert.alert('Missing schedule', 'Choose a scheduled date and pickup time before booking.');
      return;
    }
    ...
    try {
      ...
      const normalizedVehicleName = selectedVehicle.name.trim().toLowerCase();
      let pricing = passedVehiclePricing.find(
        (row) => row.vehicle_type.trim().toLowerCase() === normalizedVehicleName && row.is_active
      ) ?? null;

      if (!pricing) {
        // Exact eq() filter can miss rows edited in Studio with different casing/whitespace,
        // so fetch all active rows and match the same way as the local fallback above.
        const { data: activePricingRows, error: pricingError } = await supabase
          .from('vehicle_pricing')
          .select('id, vehicle_type, base_fare, price_per_km, price_per_min, delay_price_per_min, vat_percentage, state_levy, is_active')
          .eq('is_active', true)
          .returns<VehiclePricingRow[]>();
        ...
        if (__DEV__) {
          console.log('[ScheduledBooking] Active vehicle_pricing rows:', activePricingRows);
        }

        pricing =
          activePricingRows?.find((row) => row.vehicle_type.trim().toLowerCase() === normalizedVehicleName) ??
          null;
      }

      if (!pricing) {
        throw new Error(`${selectedVehicle.name} pricing is unavailable for this booking right now.`);
      }

      const fareBreakdown = computeFareBreakdown(pricing, totalKm ?? 0, totalTime ?? 0);
      const totalKmValue = typeof totalKm === 'number' && Number.isFinite(totalKm) ? totalKm : null;
      const totalTimeValue = routeMetrics?.etaText?.trim()
        ? routeMetrics.etaText.trim()
        : typeof totalTime === 'number' && Number.isFinite(totalTime) && totalTime > 0
          ? `${totalTime} min`
          : null;
      ...
      const parsedPassengerCount = Number.parseInt(resolvedPassengerCount, 10);
      const passengerNum = Number.isFinite(parsedPassengerCount)
        ? Math.max(1, Math.min(parsedPassengerCount, 4))
        : 1;

      const nextBookingStatus = 'pending' as const;
      const untypedSupabase = supabase as unknown as {
        from: (table: 'schedule_booking') => {
      ...
      const scheduleBookingPayload: Record<string, unknown> = {
        rider_id: profile.uuid,
        schedule_type: resolvedScheduleType,
        schedule_date: resolvedScheduledDate,
        pickup_time: resolvedScheduledTime,
      ...
      const { data: createdBooking, error } = await untypedSupabase
        .from('schedule_booking')
        .insert(scheduleBookingPayload)
        .select('id')
        .maybeSingle();
      ...
      if (createdBooking?.id) {
        const { error: notifyError } = await supabase.functions.invoke('notify-rider-scheduled-booking-created', {
          method: 'POST',
          body: {
        ...
        if (notifyError) {
          console.warn('[ScheduledBooking] Rider registration notification failed', notifyError);
        }
      }

      if (nextBookingStatus === 'pending') {
        try {
          ...
          console.warn('[ScheduledBooking] Saved route cache failed', storageError);
        }
      }
      ...
      console.error('[ScheduledBooking] Create failed', error);

      Alert.alert(
        'Unable to create scheduled booking',
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.mapContainer}>
        ...
        {resolvedScheduledDate && resolvedScheduledTime ? (
          <View style={[styles.scheduleBadge, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}> 
            <Ionicons name="calendar-outline" size={14} color={theme.colors.primary} />
            <Text style={[styles.etaBadgeText, { color: theme.colors.text }]}>
              {resolvedScheduledDate} • {formattedScheduledTime}
              {resolvedPassengerCount ? ` • ${resolvedPassengerCount} passenger${resolvedPassengerCount === '1' ? '' : 's'}` : ''}
            </Text>
          </View>
        ) : null}
      </View>
                ...
                <View style={styles.vehiclePriceContainer}>
                  {vehicle.badge ? (
                    ...
                    <Text style={[styles.vehiclePrice, { color: theme.colors.text }]}> 
                      ₦{(vehiclePriceEstimates[vehicle.name] ?? vehicle.priceNaira).toLocaleString()}
                    </Text>
                  )}
                </View>
              </TouchableOpacity>
        ...
        <View style={styles.paymentRow}>
          <View style={styles.paymentTitleRow}>
            {paymentMethod === 'wallet' ? (
              <TouchableOpacity onPress={openQuickWallet} style={styles.paymentTitleIconButton}>
        ...
        >
          <Text style={styles.bookButtonText}>
            {isCreatingBooking
              ...
              : `Book ${selectedVehicle.name} - ₦${(vehiclePriceEstimates[selectedVehicle.name] ?? selectedVehicle.priceNaira).toLocaleString()}`}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      >
        <View style={styles.successModalBackdrop}>
          <View style={[styles.successModalCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}> 
            ...
            />
            <Text style={[styles.successTitle, { color: theme.colors.text }]}>Scheduled Book Successfully</Text>
            <Text style={[styles.successSubtitle, { color: theme.colors.textSecondary }]}>Your ride captain will be with you shortly</Text>
          </View>
        </View>
      </Modal>
    </View>
